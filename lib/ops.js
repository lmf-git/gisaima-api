...
// active battles, or a structure (passive production, building, recruitment).
// Any non-null write to one of those tile sub-paths promotes the chunk to
// `active:true` so the next tick loads it. Demotion (→ false) is owned solely
// by the tick's reconcile pass, never here — Ops can't see whole-chunk state,
// so it only ever promotes (the conservative, no-false-negative direction).
const WORK_PATH = /^tiles\.[^.]+\.(groups|battles|structure)(\.|$)/;
...
export class Ops {
  ...
  async flush(db) {
    ...
    for (const report of this._reports) {
      ops.push(db.collection('reports').insertOne(report));
    }
